...
public class SaturationArithmetic
{
    ...
    public void emitSaturatedAdd(String dst, String src1, String src2)
    {
        ...
        gen.emit(String.format("li $at, %d", MAX_VALUE));
        gen.emit(String.format("bgt %s, $at, %s", dst, labelMax));

        // Check underflow (result < MIN_VALUE)
        gen.emit(String.format("li $at, %d", MIN_VALUE));
        gen.emit(String.format("blt %s, $at, %s", dst, labelMin));
    ...
    public void emitSaturatedNeg(String dst, String src)
    {
        ...
        gen.emit(String.format("li $at, %d", MIN_VALUE));
        gen.emit(String.format("beq %s, $at, %s", src, labelSaturate));
